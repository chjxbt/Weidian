      ...
      <div class="tab-list">
        <el-tabs v-model="activeName" @tab-click="handleClick">
          <div v-for="item in tabList">
            <el-tab-pane :label="item" :name="item.slice(0, 3)" :lazy="lazyStatus">{{item}}</el-tab-pane>
          </div>
        </el-tabs>
      </div>
  ...
  export default {
    data() {
        return {
          ...
          logisticsNo: "",              // 物流单号
          buyerTel: "",                 // 收货人电话
          activeName: "全部",           // tabs标签默认选中的项
          tabList: ["全部"],            // tabs标签的list
        ...
        axios.get(api.get_order_count + '?token=' + localStorage.getItem('token')).then(res => {
          if(res.data.status == 200) {
            ...
            for(let i = 0; i < res.data.data.length; i ++) {
              if(res.data.data[i].status == "全部") {
                this.tabList[0] = "全部";
              }else {
                this.tabList.push(res.data.data[i].status + "(" + res.data.data[i].count + ")");
              }
            }
          }else{
