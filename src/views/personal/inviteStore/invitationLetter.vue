      ...
      <div class="m-fans-img-modal" @click="closeFansModal" v-if="show_fans_img">
        <div class="m-modal-state" @click.stop="selectImg">
          ...
          <div class="m-headPortrait-name">
            <span class="m-head-name">{{usname}}</span>
            <img :src="usheader" class="m-head-portrait" />
          </div>
          <mt-popup
            class="help-popup"
            ...
            <img style="width: 100%;height: 100%;" :src="card_src" alt="" >
          </mt-popup>
        </div>
      </div>
        ...
        <div class="m-modal-state">
          <div class="m-modal-content">
            ...
            <div class="m-fans-btn">
              ...
              <span class="active" @click="selectImg">好的</span>
            </div>
          </div>
        </div>
      </div>
    ...
    export default {
      ...
      mounted(){
        ...
        methods: {
          getInfo(){
            axios.get(api.get_image_by_aitype,{
              ...
              if(res.data.status == 200){
               this.card_src = res.data.data.aiimage;
              }else{
              }
            },error => {

            })
            axios.get(api.get_one_reward,{
              params:{
                token:localStorage.getItem('token'),
                urid:this.$route.query.urid
              }
            }).then(res => {
              if(res.data.status == 200){
                this.card_info = res.data.data;
                this.usheader =res.data.usheader;
          ...
          selectImg(){
            axios.post(api.get_transfer_reward+'?token='+localStorage.getItem('token'),{
              urid:this.$route.query.urid,
              usid:this.$route.query.usid
            }).then(res => {
              if(res.data.status == 200) {
                this.show_fans_img = false;
                this.show_fans =false;
                this.helpPopupVisible =false;
