...
def list_add_models(model_name, items):
    ...
    try:
        for item in items:
            ...
            lower_key = list(map(lambda x: x.lower(), model_bean_key))   # 数据库的字段转小写
            for item_key in item.keys():
                import ipdb
                # ipdb.set_trace()
                if item_key.lower() in lower_key:  # 如果json中的key同时也存在与数据库的话
                    # 找到此key在model_beankey中的位置
                    index = lower_key.index(item_key)
        ...
        print 'sorry we make an mistake'
...
if __name__ == '__main__':
    ...
    fix_to_add_models('Product', items)
