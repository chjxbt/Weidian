...
class SProductImage(SBase):
    ...
    def get_images_by_prid_pisort(self, prid, pisort):
        return self.session.query(ProductImage).filter_by(PRid=prid, PIsort=pisort).order_by(
            ProductImage.PIsort).first()

    @close_session
    def update_image(self, piid, piimage):
        return self.session.query(ProductImage).filter(ProductImage.PIid == piid).update(piimage)